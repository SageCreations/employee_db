...
int add_employee(struct dbheader_t *dbhdr, struct employee_t **employees, char *addstring) {
	if (dbhdr == NULL) {
		printf("Database header was null when adding an employee.\n");
		return STATUS_ERROR;
	}
	// if (employees == NULL || *employees == NULL) {
	if (employees == NULL) {
		printf("In memory employee data was null.\n");
		return STATUS_ERROR;
	}
	if (addstring == NULL) {
		return STATUS_ERROR;
	...
	// increase memory size for the new employee
	*employees = realloc(*employees, (dbhdr->count)*sizeof(struct employee_t));
	if (*employees == NULL) {
		printf("realloc for employees failed");
		return -1;
	}
	...
	if (name == NULL || address == NULL || hours == NULL) {
		printf("Input for new employee was not correct.\n");
		return STATUS_ERROR;
	}
